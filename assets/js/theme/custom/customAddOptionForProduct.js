...
export default function(context, wrapper) {
    if (context.themeSettings.customAddOptionForProduct == true) {
        ...
        function renderOption(data){
            ...
            $.each(aFilter, (index, element) => {
                ...
                var swatchCard = product_wrapper.find('.cardSwatch_count-'+productId+' [data-swatch-count]');
                ...
                $.each(aFilter2, (idx, el) => {
                    ...
                    if(aFilter4.length > 0){
                        ...
                        var optionID = aFilter4[0].node.entityId;
                        ...
                        $.each(aFilter5, (idx, element) => {
                            ...
                            const cardLink =  productFieldColor.closest('.card').find('.card-title a').attr('href');
                            
                            if(lengthColorVar == 2){
                                productFieldColor.append(`
                                    <a href="${cardLink}?variant=attribute_swatch_${optionID}_${idVar}" class="form-option form-option-swatch two-colors" data-image="${variantImage}" data-product-swatch-value="${idVar}" data-title="${titleVar}">
                                        <span class="form-option-tooltip">${titleVar}</span>
                                        <span class="form-option-variant form-option-variant--color form-option-variant--color2" title="${titleVar}">
                                            <span class="form-option-variant form-option-variant--color two-colors" style="background-color:${color1};border: none;left: -1px;"></span>
                                            <span class="form-option-variant form-option-variant--color two-colors" style="background-color:${color2};border: none;"></span>
                                        </span>
                                    </a>
                                `);

                                if(countSwatch > 0) {
                                    swatchCard.text(`${countSwatch} colors`);
                                }

                            } else if(lengthColorVar === 3){
                                productFieldColor.append(`
                                    <a href="${cardLink}?variant=attribute_swatch_${optionID}_${idVar}" class="form-option form-option-swatch three-colors two-colors" data-image="${variantImage}" data-product-swatch-value="${idVar}" data-title="${titleVar}">
                                        <span class="form-option-tooltip">${titleVar}</span>
                                        <span class="form-option-variant form-option-variant--color form-option-variant--color2" title="${titleVar}">
                                            <span class="form-option-variant form-option-variant--color three-colors" style="background-color:${color1}"></span>
                                            <span class="form-option-variant form-option-variant--color three-colors" style="background-color:${color2}"></span>
                                ...
                                productFieldColor.append(`
                                <a href="${cardLink}?variant=attribute_swatch_${optionID}_${idVar}" class="form-option form-option-swatch" data-image="${variantImage}" data-product-swatch-value="${idVar}" data-title="${titleVar}">
                                    <span class="form-option-tooltip">${titleVar}</span>
                                ...
                                productFieldColor.append(`
                                    <a href="${cardLink}?variant=attribute_swatch_${optionID}_${idVar}" class="form-option form-option-swatch" data-image="${variantImage}" data-product-swatch-value="${idVar}" data-title="${titleVar}">
                                        <span class="form-option-tooltip">${titleVar}</span>
                                        <span class="form-option-variant form-option-variant--pattern" title="${titleVar}" style="background-image: url(${img})"></span>
                                    </a>
                                `);
